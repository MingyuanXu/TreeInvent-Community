...
for temp in tqdm(np.arange(1,5.01,0.05)):
    ...
    img=Draw.MolsToGridImage(mols,molsPerRow=5,subImgSize=(200,250),legends=list([f'{float(a):.3F}' for a in sims]),useSVG=True)
    with open(f'{picpath}/ft_temp_{temp:.2E}.svg','w') as f:
        f.write(img)
